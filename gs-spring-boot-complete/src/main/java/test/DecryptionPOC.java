...
public class DecryptionPOC {
	public static void main(String[] args) throws Exception {
	    ...
	    System.out.println(plaintextString + " " + bytesToHex(plaintextString.getBytes()).toLowerCase() + " " + Arrays.toString(plaintextString.getBytes()));
	    ...
	    System.out.println("Length: "+ secretkey.getEncoded().length);
	    Cipher cipher = Cipher.getInstance("Blowfish");

	    cipher.init(Cipher.ENCRYPT_MODE, key);
	    byte[] encrypted = cipher.doFinal(plaintextString.getBytes());
	    String encryptedString = bytesToHex(encrypted);
	    System.out.println("Encrypted: "+ new String(encrypted) + " " + encryptedString + " " + Arrays.toString(encrypted));
